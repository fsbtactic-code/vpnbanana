...
Замер пула к целям идёт **параллельно** (`PROBE_PARALLEL`, по умолчанию **12**, чтобы реже ловить таймауты и «ложный» лидер при перегрузе исходящих соединений). Затем **вторая волна**: у **топ-8** кандидатов из первой сортировки делаются **3** последовательных замера на хост, берётся **медиана** (`VERIFY_TOP_N`, `VERIFY_SAMPLES`). Хосты с невалидным сертификатом под имя отбрасываются (`REALITY_SSL_VERIFY=1` по умолчанию, как для публичного dest REALITY). Полный лог 1-й волны: `POOL_PROBE_FULL=1`. Только последовательная 1-я волна: `PROBE_PARALLEL=1`. Отключить перепроверку: `VERIFY_TOP_N=0`. Старый `curl` без `%{ssl_verify_result}`: `REALITY_SSL_VERIFY=0`.
